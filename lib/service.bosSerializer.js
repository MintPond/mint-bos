...
function _writeUInt64(val) {

    _ensureBuffer(9);
    size += 9;

    // DataType
    buffer.writeUInt8(DataType.UINT64, pos);
    pos += 1;

    const valBi = BigInt(val);
    const left = (valBi >> (4n * 8n)) & BigInt(0xFFFFFFFF);
    const right = valBi & BigInt(0xFFFFFFFF);

    const biBuffer = Buffer.alloc(8);
    biBuffer.writeUInt32LE(parseInt(right), 0);
    biBuffer.writeUInt32LE(parseInt(left), 4);
    biBuffer.copy(buffer, pos);
    pos += 8;
}
